    ...
    <>
      {films.map((film) => {
        ...
        if (film.film_description.length > 1000) {
          descript = `${film.film_description.slice(
            0,
            1000
          )} ... (для полного оаисания нажмите на текст)`;
        } else {
          ...
          <div key={film.id} className="film-card">
            <div className="film-side">
              ...
              <div className="film-all-discription">
                ...
                <h2
                  className="film-discription"
                  onClick={() => toggleDescription(film.id)}
                >
                  {descript}
                </h2>
                <h2 className="film-duration">
                  {film.film_duration} минут {film.film_origin}
                </h2>
              </div>
            </div>
            <CardHall filmSeance={filmSeance} halls={halls} />
          </div>
        );
      })}
    </>
