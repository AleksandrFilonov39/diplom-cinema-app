...
function ConfigHall({allData}) {
    ...
    function updateRowPalce (e) {
        ...
        if(currentHall.places > 15 || currentHall.places <= 0) {
            alert('число мест в ряду не должно превышать 15 и не может бать меньше нуля')
            setCurrentHall(prev => ({...prev, places: 10}))
            return;
        }

        if(currentHall.rows > 15 || currentHall.rows <= 0) {
            alert('число рядов в зале не должно превышать 15 и не может бать меньше нуля')
    ...
    function chengePlaсe(indRow, indPlace) {
        ...
        row = [
            ...row.slice(0, indPlace),
            placeName,
            ...row.slice(indPlace + 1)
        ];
        arr = [
            ...arr.slice(0, indRow),
            row,
    ...
    const handleClick = async () => {
        ...
        try {
         const response = await fetch(`https://shfe-diplom.neto-server.ru/hall/${currentHall.id}`, {
            method: 'POST',
            body: params
        })
    ...
    <div className='wrp-config-hall'>
       <h2 className='config-hall-title'>Выберите зал для конфигурации:</h2>
        <HallsUI 
            ...
            id={currentHall.id}
       />
       <h2 className='config-hall-title'>Укажите количество рядов и максимальное количество кресел в ряду:</h2>
       <form className='config-hall-form'>
        <label htmlFor="rows" className='form-addHall-title'>
            <p className='form-addHall-text'>Рядов, шт</p>
            <input type="number" name='rows' className='form-addHall-input' value={currentHall.rows} onChange={updateRowPalce}/>
        </label>
       ...
       <h2 className='title-seats-text'>Чтобы изменить вид кресла, нажмите по нему</h2>
