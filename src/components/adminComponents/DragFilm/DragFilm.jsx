...
function DragFilm({data, deleteFilm, onDragStart, setBgColor}) {
  ...
  return (
    <div className='wrp-drag-films'>
      {data.result.films.map((film) => (
        <div key={film.id} style={{backgroundColor: getRandomHexColor(film.id)}} className='wrp-drop-film' draggable='true' onDragStart={(e) => handleDragStart(e, film)}>
          ...
          <button className='drop-film-delete-btn' onClick={() => deleteFilm(film.id)}></button>
        </div>
      ))}
    </div>
  )
