...
function Calendar() {
  ...
  const chengeActive = (e, index, day, month) => {
    e.preventDefault();
    const newDate = `${new Date().getFullYear()}-${String(month + 1).padStart(
      2,
      "0"
  ...
  return (
    <Slider {...settings} className="wrp-carusel">
      {date.daysToShow.slice(0, 14).map((day, index) => {
        const isToday =
          day[0] === date.dayOfMonth && day[1] === date.dayOfWeekName;
        ...
        return (
          <div key={`${day[0]}-${day[1]}-${index}`} className="calendaer__list">
            <a
              className={`calendaer__link ${isToday ? "today" : ""} ${
                isWeecand ? "weecand" : ""
              } ${active ? "active" : ""}`}
