  ...
  return (
    <>
       {halls.map((hall) => {
        ...
        return (
           <div key={hall.id}>
                <h1 className='film-hall'>
                    {hall.hall_name}
                </h1>
                <div className='film-wrp-allTime'>
                    {seance.map((el) => {
                      const timeSeance = el.seance_time.split(':').map(el => el.padStart(2, '0')).join(':');
                      let currentSeances = true;
                      if (dateNow === selectedDate && time > timeSeance){
                        currentSeances = false;
                      };
                      return ( 
                      <Link to={currentSeances ? `/hallconfig/${el.id}/${selectedDate}` : '#'} className={`film-time ${!currentSeances ? 'disabled' : ''}`} key={el.id}>
