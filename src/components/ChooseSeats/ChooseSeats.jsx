...
const filmName = allData.result.films.find((el) => el.id === seance.seance_filmid)
const hallName = allData.result.halls.find((el) => el.id === seance.seance_hallid)
...
const chooseSeats = (seat, row, place) => {
  if (seat === 'taken' || seat === 'disabled') {
   ...
   const seatIndex = choosenSeats.findIndex((ele) => ele[0] === row && ele[1] === place)
   console.log(seatIndex, 'seatIndex')
  if (seatIndex !== -1) {
    setChoosenSeats([...choosenSeats.filter((ele, index) => index !== seatIndex)]);
  } else {
...
const handleClick = () => {
  ...
  setChoosenSeats([...choosenSeats.map((el) => el.push(data.result[el[0]][el[1]]  === 'vip'? hallName.hall_price_vip : hallName.hall_price_standart))]);
  navigate('/buyTickets', {
     replace: true,
     state: {
      choosenSeats,
      ...
      <div className='wrp-title-choose-seats'>
        <div className='wrp-title-choose'>
          <div className='wrp-title-choose-name'>
            ...
            <p className='title-choose-seats-film-time'>Начало сеанса: {seance.seance_time}</p>
          ...
          <div className='wrp-hall-choose-seats'>
           ...
           <div className='hall-seats-choose'>
            {data.result?.map((row, indRow) => (
              <div className='hall-seats-choose-row' key={indRow}>
                {row.map((el, indPlace) => (
                  <span className={choosenSeats.find((ele) => ele[0] === indRow && ele[1] === indPlace)? 'seats-choose-select' : `seats-choose-${el}`} key={indPlace} onClick={() => chooseSeats(el, indRow, indPlace)}><p className='seats-choose-name'></p></span>
                ))}
              </div>
            ))    
            }
           ...
           <div className='seats-choose-varaibles'>
            <div className='seats-choose-varaibles-standart'>
              ...
              <p className='seats-choose-name'>Свободно ({hallName.hall_price_standart}руб.)</p>
            </div>
            <div className='seats-choose-varaibles-standart'>
            ...
            <div className='seats-choose-varaibles-standart'>
                ...
                <p className='seats-choose-name'>  Свободно VIP ({hallName.hall_price_vip}руб.)</p>
            </div>
            <div className='seats-choose-varaibles-standart'>
